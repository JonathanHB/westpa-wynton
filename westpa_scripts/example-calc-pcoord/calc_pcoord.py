# Jonathan Borowsky
# progress coordinate calculation for UCP1-DNP simulations based on 
# DNP z coordinate relative to UCP1 center
# and conformational changes
# calc_dnp_pc_conf_z_v1
...
def get_circmean_coordinates(frame, query, box_lengths):
    ...
    circmean_xyz = [
        np.arctan2(np.mean([np.sin(a) for a in atom_angles[j]]), np.mean([np.cos(a) for a in atom_angles[j]]))
...
def get_distance_matrix(state1, resseqs, periodic):
    # c-alpha indices
    prot_ca_state1 = [a.index for a in state1.top.atoms if
                      a.residue.is_protein and a.name == 'CA' and a.residue.resSeq in resseqs]
    ...
    distances_state1 = md.compute_distances(state1, pairs_state1, periodic=periodic, opt=True).reshape(n_ca_state1,
                                                                                                       n_ca_state1)
...
ucp1_cyt_resseqs_8j1n   = [92,  95,  96,  99,  191, 194, 195, 198, 285, 288, 289, 292] #for 8j1n numbering
ucp1_cyt_resseqs        = [i-resishift for i in ucp1_cyt_resseqs_8j1n] #for gro file numbering
...
ucp1_mat_resseqs_8j1n = [34, 37, 38, 134, 137, 138, 233, 236, 237] #for 8j1n numbering
ucp1_mat_resseqs      = [i-resishift for i in ucp1_mat_resseqs_8j1n] #for gro file numbering


#print("+".join([str(r) for r in bov_cytoplasmic_resseqs]))
#print("+".join([str(r) for r in ucp1_cyt_resseqs]))

#print("+".join([str(r) for r in bov_matrix_resseqs]))
#print("+".join([str(r) for r in ucp1_mat_resseqs]))
...
if mode == 0:
    ...
    ref_inputpath = "/wynton/home/grabe/jborowsky/aac1/westpa-13/westpa_scripts/pcoord-ref-structures"
    ...
    frame = md.load(f"{inputpath}/trajectories/solution-dnp/we06/000013-000039/seg.gro")
...
cyt_pc = calc_progress_coord_component(bov_cstate_structure, fun_mstate_structure, frame, bov_cytoplasmic_resseqs,
                                       fun_cytoplasmic_resseqs, ucp1_cyt_resseqs, ref_state)
mat_pc = calc_progress_coord_component(bov_cstate_structure, fun_mstate_structure, frame, bov_matrix_resseqs,
                                       fun_matrix_resseqs, ucp1_mat_resseqs, ref_state)
